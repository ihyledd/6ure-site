...
function usePlatform() {
  ...
  useEffect(() => {
    ...
    const isMobile = /iPhone|iPad|iPod|Android/i.test(ua) || ("ontouchstart" in window && window.innerWidth < 768);
    if (isMobile) {
      setPlatform("mobile");
...
function SearchTrigger({ onOpen }: { onOpen: () => void }) {
  ...
  return (
    <button type="button" onClick={onOpen} className="search-trigger">
      <SearchIcon />
...
export function SearchBox() {
  ...
  const overlay = open && typeof document !== "undefined"
    ? createPortal(
        <SearchOverlayInner onClose={() => setOpen(false)} />,
        document.body,
      )
    : null;
