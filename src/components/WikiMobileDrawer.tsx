...
function NavIcon({ d, children }: { d?: string; children?: React.ReactNode }) {
...
export function WikiMobileDrawer({ discordUrl, discordLoginUrl, hasAdmin, hasSession, requestsSubLinks }: Props) {
  ...
  const navItems = [
    { href: "/requests", label: "Requests", icon: <NavIcon d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" /> },
    { href: "/resources", label: "Resources", icon: <ResourcesIcon /> },
    { href: "/password", label: "Password", icon: <NavIcon d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4" /> },
  ];
    ...
    <div className="wiki-mobile-drawer-root" role="dialog" aria-modal="true" aria-label="Menu">
      ...
      <aside className="wiki-mobile-drawer-panel">
        ...
        <nav className="wiki-mobile-drawer-nav">
          ...
          {hasAdmin && (
            <label htmlFor="ure-mobile-nav-toggle" className="wiki-mobile-drawer-item">
              <Link href="/admin" className="wiki-mobile-drawer-link">
