...
const DEFAULT_DISCORD_INVITE = "https://discord.gg/wFPsTezjeq";
...
function LandingView({ discordUrl }: { discordUrl: string }) {
  return (
    <div className="v2-verify">
      <div className="v2-verify-card v2-landing">
        ...
        <a href={discordUrl} className="v2-cta" target="_blank" rel="noopener noreferrer">
          <DiscordIcon size={20} />
          Join 6ure Discord
        </a>
...
function VerifyFromOAuthCode({ code, discordUrl }: { code: string, discordUrl: string }) {
  ...
  return <VerificationView userId={userId} discordUrl={discordUrl} />;
}

function VerificationView({ userId, discordUrl }: { userId: string, discordUrl: string }) {
...
export function VerifyPage({ discordUrl = DEFAULT_DISCORD_INVITE }: { discordUrl?: string }) {
  const invite = discordUrl;
  ...
  return (
    <div className="verify-page-wrap">
      ...
      {userId ? (
        <VerificationView userId={userId} discordUrl={invite} />
      ) : code ? (
        <VerifyFromOAuthCode code={code} discordUrl={invite} />
      ) : (
        <LandingView discordUrl={invite} />
      )}
    </div>
  );
}
