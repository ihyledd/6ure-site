...
type PopupType = "discord" | "leaked" | "protected" | "preview" | "upsell" | "cancel" | "error";
...
export function PopupPreview({ type, data }: { type: PopupType; data: PopupData }) {
  ...
  if (type === "error") {
    return (
      <div className="popup-preview-wrapper">
        <span className="popup-preview-label">Preview</span>
        <div className="popup-preview-mock" style={{ textAlign: "center" }}>
          <div style={{ fontSize: 32, color: "#f59e0b", marginBottom: 8 }}>⚠</div>
          <h4 style={{ marginBottom: 4 }}>Service Unavailable</h4>
          <p style={{ maxWidth: 360, margin: "0 auto 12px", fontSize: 13, lineHeight: 1.5, opacity: 0.7 }}>
            {g(data, "popup_error_api_offline", "The requests service is currently unavailable. Please try again later.")}
          </p>
          <span className="popup-preview-btn popup-preview-btn-primary" style={{ fontSize: 12 }}>Try Again</span>
        </div>
      </div>
    );
  }
