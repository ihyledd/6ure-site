...
export async function syncRequestsUser(
  ...
  const hasVerifiedRole = verifiedRoleId ? roles.includes(verifiedRoleId) : true;

  // Some deployments may not have `leak_protection` or `verified` yet. Try full upsert; fall back if columns missing.
  try {
    await execute(
      `INSERT INTO users (id, username, discriminator, global_name, display_name, avatar, banner, accent_color, public_flags, premium_type, roles, patreon_premium, leak_protection, verified, guild_nickname, guild_avatar, boost_level, premium_since, avatar_decoration, created_at, updated_at, last_login_at, last_activity_at)
    ...
    // Fallback for older schemas (missing verified or leak_protection)
    await execute(
