...
/** Notify bot that a request was fulfilled — DM creator + upvoters. */
export function notifyFulfillment(params: {
  requestId: number;
  requestTitle: string;
  requestUrl: string;
  leakMessageUrl?: string | null;
  creatorUserId: string;
  upvoterUserIds: string[];
}): void {
  post("/notify-fulfillment", params).catch((err) =>
    logBotError("/notify-fulfillment", err)
  );
}

/** Check if a user is in the Discord server. */
export async function isUserInServer(userId: string): Promise<boolean> {
  try {
    const res = await get(`/is-member/${userId}`, 4000);
    if (!res.ok) return false;
    const data = await res.json() as { isMember: boolean };
    return !!data.isMember;
  } catch (err) {
    logBotError(`/is-member/${userId}`, err);
    // If bot is down, we might want to fail-safe?
    // User wants "they need to be in our server", so we should probably return false if we can't verify.
    return false;
  }
}
