...
function rowToRequestData(row: Record<string, unknown>): RequestData {
  ...
  const isAnon = row.anonymous === 1 || row.anonymous === true;
  const username = isAnon ? "Anonymous" : (row.username as string) || "Anonymous";
  const avatar = isAnon ? null : (row.avatar as string | null) ?? null;
  const avatarDecoration = isAnon ? undefined : (row.avatar_decoration as string) ?? undefined;
...
export async function getRequestById(id: number): Promise<RequestData | null> {
  const row = await queryOne<Record<string, unknown>>(
    `SELECT r.*,
            ...
            CASE WHEN r.anonymous = 1 THEN NULL ELSE u.avatar_decoration END as avatar_decoration,
  ...
  if (!row) return null;
  let data = rowToRequestData(row);
  data = await autoCompleteIfLeaked(data);
  return data;
}

export async function getRequestByIdForBot(
  id: number
): Promise<{ user_id?: string; title?: string; product_url?: string; message_id?: string; public_message_id?: string; leak_message_url?: string } | null> {
  const row = await queryOne<Record<string, unknown>>(
    "SELECT user_id, title, product_url, message_id, public_message_id, leak_message_url FROM requests WHERE id = ?",
    [id]
  );
  if (!row) return null;
  return {
...
export async function getRequestsList(opts: {
  ...
  // Run main query, count query, and leak index pre-warm in parallel
  ...
  let requests = rows.map(rowToRequestData);
  // Auto-complete any pending requests that have already been leaked
  requests = await Promise.all(requests.map(autoCompleteIfLeaked));
  return {
    requests,
...
/**
 * Look up a leak by product URL from the YAML leak data files.
 */
export async function getLeakByProductUrl(
  productUrl: string
): Promise<LeakInfo | null> {
  const entry = await findLeakByProductUrl(productUrl);
...
/**
 * If a request is pending and its product URL matches a leaked entry,
 * auto-update status to 'completed' and set the leak_message_url.
 */
async function autoCompleteIfLeaked(request: RequestData): Promise<RequestData> {
  if (request.status !== "pending") return request;
  if (!request.product_url) return request;
  try {
    const entry = await findLeakByProductUrl(request.product_url);
    if (!entry) return request;
    const leakMessageUrl = entry.discordMessageUrl || null;
    // Update DB
    const setClauses = ["status = 'completed'"];
    const params: unknown[] = [];
    if (leakMessageUrl && !request.leak_message_url) {
      setClauses.push("leak_message_url = ?");
      params.push(leakMessageUrl);
    }
    params.push(request.id);
    await execute(
      `UPDATE requests SET ${setClauses.join(", ")} WHERE id = ? AND status = 'pending'`,
      params
    );
    return {
      ...request,
      status: "completed",
      leak_message_url: leakMessageUrl || request.leak_message_url,
    };
  } catch (e) {
    console.warn("[AutoComplete] Failed to check leak for request", request.id, e);
    return request;
  }
}
...
export async function getUpvoters(
  ...
  upvoters: Array<{ user_id: string; username: string; avatar: string | null; avatar_decoration: string | null }>;
  ...
  const rows = await query<{
    ...
    `SELECT u.id, u.username, u.global_name, u.avatar, u.avatar_decoration
     FROM upvotes up
  ...
  const upvoters = rows.map((r) => {
    ...
    return {
      ...
      username,
      avatar,
      avatar_decoration: r.avatar_decoration ?? null,
    };
  });
...
/** Get just the user IDs of all upvoters for a request (for DM notifications). */
export async function getUpvoterUserIds(requestId: number): Promise<string[]> {
...
// --- Protected user check (users who are protected editors cannot submit requests) ---

export async function isProtectedUser(userId: string): Promise<boolean> {
  const row = await queryOne<{ leak_protection: number | boolean }>(
    "SELECT COALESCE(leak_protection, 0) as leak_protection FROM users WHERE id = ?",
    [userId]
  );
  return Boolean(row?.leak_protection);
}

// --- Daily request limit ---

export async function getRequestCountByUserInLast24Hours(
  userId: string
): Promise<number> {
  const [row] = await query<{ cnt: number }>(
    "SELECT COUNT(*) as cnt FROM requests WHERE user_id = ? AND created_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR)",
    [userId]
  );
  return Number(row?.cnt ?? 0);
}

// --- User info from DB (roles, premium) ---

export async function getUserFromDb(
  userId: string
): Promise<{ roles: string | null; patreon_premium: boolean } | null> {
  const row = await queryOne<{ roles: string | null; patreon_premium: number }>(
    "SELECT roles, patreon_premium FROM users WHERE id = ?",
    [userId]
  );
  if (!row) return null;
  return {
