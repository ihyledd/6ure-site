 ...
 * Post-checkout redirect. Captures lifetime orders and activates subscriptions.
 * Query params: type (subscription|lifetime), category, interval?
 */
...
export async function GET(request: Request) {
  ...
  if (!session?.user?.id) {
    return NextResponse.redirect(`${SITE_URL}/membership?error=auth`);
  }
  ...
  const token = url.searchParams.get("token"); // PayPal sends this for orders
  const subscriptionId = url.searchParams.get("subscription_id"); // PayPal sends this for subscriptions
  ...
  try {
    if (type === "lifetime" && token) {
      ...
      const sub = await queryOne<{ id: string; amount: string; plan_category: string }>(
        `SELECT id, amount, plan_category FROM subscriptions WHERE paypal_order_id = ? AND user_id = ?`,
      ...
      if (sub) {
        await execute(
          `UPDATE subscriptions
           SET status = 'ACTIVE',
               ...
               updated_at = NOW()
           WHERE id = ?`,
          [payerEmail, payerName, sub.id]
        );

        // Record payment
        const paymentId = `pay_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
        await execute(
          `INSERT INTO payments (id, subscription_id, paypal_transaction_id, paypal_sale_id, amount, currency, status, payer_email, payer_name, created_at)
        ...
        await execute(
          `UPDATE promo_codes pc
           ...
           WHERE s.id = ? AND s.promo_code_id IS NOT NULL`,
          [sub.id]
        );
      ...
      return NextResponse.redirect(
        `${SITE_URL}/membership?success=true&category=${category}&interval=LIFETIME`
      );
    }
    ...
    if (type === "subscription" && subscriptionId) {
      ...
      const payerName = details?.subscriber?.name
        ? `${details.subscriber.name.given_name ?? ""} ${details.subscriber.name.surname ?? ""}`.trim()
        : null;
      ...
      if (details.status === "ACTIVE" || details.status === "APPROVED") {
        ...
        await execute(
          `UPDATE promo_codes pc
           JOIN subscriptions s ON s.promo_code_id = pc.id
           SET pc.used_count = pc.used_count + 1
           WHERE s.paypal_subscription_id = ? AND s.promo_code_id IS NOT NULL`,
          [subscriptionId]
        );
      }

      return NextResponse.redirect(
        `${SITE_URL}/membership?success=true&category=${category}&interval=${interval}`
      );
    }
