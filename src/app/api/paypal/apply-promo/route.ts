/**
 * POST /api/paypal/apply-promo
 * Validates a promo code and returns discount info.
 * Body: { code, planCategory }
 */
...
export async function POST(request: Request) {
  const { code, planCategory } = (await request.json()) as {
    code: string;
    planCategory: string;
  };
  ...
  const promo = await queryOne<{
    ...
    `SELECT id, code, discount_percent, max_uses, used_count, valid_from, valid_until, plan_category, active
  ...
  if (promo.plan_category && promo.plan_category !== planCategory) {
    return NextResponse.json({ valid: false, error: "This code is not valid for this plan" });
  }
