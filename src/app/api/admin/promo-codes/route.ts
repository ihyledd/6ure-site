...
export async function GET() {
  ...
  const codes = await query(
    `SELECT pc.*,
            (SELECT COUNT(*) FROM subscriptions s WHERE s.promo_code_id = pc.id) as total_uses
     FROM promo_codes pc
...
export async function POST(request: Request) {
  ...
  await execute(
    `INSERT INTO promo_codes (id, code, discount_percent, max_uses, valid_from, valid_until, plan_category, active, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, 1, NOW())`,
    [
      id,
      code.toUpperCase(),
