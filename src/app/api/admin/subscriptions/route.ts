/**
 * GET /api/admin/subscriptions — List all subscriptions (developer only)
 */
...
export async function GET(request: Request) {
  ...
  if (category) { where += " AND s.plan_category = ?"; params.push(category); }
  if (search) {
    where += " AND (u.username LIKE ? OR s.email LIKE ? OR s.payer_name LIKE ? OR u.global_name LIKE ?)";
    const t = `%${search}%`; params.push(t, t, t, t);
  }

  const [countRow] = await query<{ total: number }>(`SELECT COUNT(*) as total FROM subscriptions s LEFT JOIN users u ON u.id = s.user_id ${where}`, [...params]);
  ...
  const subscriptions = await query(
    `SELECT s.*, u.username, u.global_name, u.display_name, u.avatar, u.guild_avatar, u.guild_nickname,
     ...
     FROM subscriptions s LEFT JOIN users u ON u.id = s.user_id ${where} ORDER BY s.created_at DESC LIMIT ? OFFSET ?`, params
  );

  return NextResponse.json({ subscriptions, total: countRow?.total ?? 0, page, limit, totalPages: Math.ceil((countRow?.total ?? 0) / limit) });
}
