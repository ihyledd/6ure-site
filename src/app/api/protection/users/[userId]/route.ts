...
export async function PATCH(
  ...
  try {
    ...
    // Re-apply auto-hide so any resources newly matching the updated creator/social link
    // are flagged as protected.
    try {
      const { applyProtectionForUser } = await import("@/lib/protection-reconcile");
      const affected = await applyProtectionForUser(uid);
      if (affected > 0) {
        console.log(`[protection/users PATCH] Auto-hid ${affected} resource(s) for ${uid}`);
      }
    } catch (e) {
      console.warn("[protection/users PATCH] applyProtectionForUser failed:", (e as Error).message);
    }
