...
export default async function EditPage({ params, searchParams }: Props) {
  ...
  async function updatePageAction(formData: FormData) {
    ...
    if (!title) {
      redirect(`/dashboard/pages/${id}/edit?error=title_required`);
      return;
    }
    ...
    try {
      ...
      redirect(`/dashboard/pages/${id}/edit?error=save_failed`);
      return;
    }
  ...
  return (
    <div style={{ marginBottom: 48 }}>
      <div className="dashboard-section-header">
        <h2>Edit page</h2>
        <p>
          <Link href="/dashboard" style={{ color: "var(--discord-blurple)" }}>Pages</Link> / Edit
        </p>
      </div>
