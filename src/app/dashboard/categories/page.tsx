  ...
  return (
    <div className="dashboard-content-block">
      ...
      <div style={{ marginBottom: 24 }}>
        <Link href="/dashboard/categories/new" className="dashboard-btn dashboard-btn-primary">
          New category
        </Link>
      </div>
      {categories.length === 0 ? (
        ...
        <ul className="dashboard-list-plain">
          {categories.map((c) => (
            <li key={c.id} className="dashboard-list-item dashboard-list-item-compact">
              <Link href={`/dashboard/categories/${c.id}/edit`} style={{ fontWeight: 600, flex: 1, minWidth: 0 }}>
                {c.name}
              </Link>
