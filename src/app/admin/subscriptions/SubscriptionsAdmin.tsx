...
export function SubscriptionsAdmin() {
  ...
  const [syncResults, setSyncResults] = useState<{ stats: any; details: any[] } | null>(null);
  ...
  const handleBulkSync = async () => {
    if (!confirm("This will attempt to assign Discord roles to ALL eligible subscribers. \n\nTo avoid rate limits, we add a 1-second delay per user. This may take a minute or more depending on subscriber count. \n\nContinue?")) return;
    ...
    try {
      const r = await fetch("/api/admin/subscriptions/bulk-sync", { method: "POST" });
      const d = await r.json();
      
      if (d.success) {
        setSyncResults({ stats: d.stats, details: d.details });
      } else {
        alert(d.error || "Failed to run bulk sync");
      }
    } catch {
  ...
  return (
    <div>
      ...
      <div style={S.tabs}>
        ...
        <button 
          style={{ ...S.actionBtn, marginLeft: "auto", color: "#a855f7", borderColor: "rgba(168,85,247,0.3)", background: "rgba(168,85,247,0.08)" }} 
          onClick={handleBulkSync}
          disabled={processingSync}
        >
          {processingSync ? "Syncing..." : "Bulk Sync Roles"}
        </button>
        <div style={{ ...S.live, marginLeft: 16 }}><div style={S.liveDot as React.CSSProperties} /> Live</div>
      </div>
      ...
      {/* SYNC RESULTS MODAL */}
      {syncResults && (
        <div style={S.refundOverlay} onClick={() => setSyncResults(null)}>
          <div style={{ ...S.refundModal, maxWidth: 600 }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h3 style={{ color: "#fff", margin: 0 }}>Sync Results</h3>
              <button style={{ ...S.actionBtn, marginRight: 0 }} onClick={() => setSyncResults(null)}>Close</button>
            </div>

            <div style={{ ...S.statsGrid, gridTemplateColumns: "repeat(3, 1fr)", marginBottom: 20 }}>
              <div style={{ ...S.statCard, padding: 12, textAlign: "center" }}>
                <div style={S.statLabel}>Success</div>
                <div style={{ ...S.statValue, color: "#4ade80", fontSize: "1.2rem" }}>{syncResults.stats.successCount}</div>
              </div>
              <div style={{ ...S.statCard, padding: 12, textAlign: "center" }}>
                <div style={S.statLabel}>Not in server</div>
                <div style={{ ...S.statValue, color: "#fbbf24", fontSize: "1.2rem" }}>{syncResults.stats.notInServerCount}</div>
              </div>
              <div style={{ ...S.statCard, padding: 12, textAlign: "center" }}>
                <div style={S.statLabel}>Errors</div>
                <div style={{ ...S.statValue, color: "#f87171", fontSize: "1.2rem" }}>{syncResults.stats.errorCount}</div>
              </div>
            </div>

            <div style={{ maxHeight: 300, overflowY: "auto", background: "#111", borderRadius: 10, padding: 12, border: "1px solid #222" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead style={{ position: "sticky", top: 0, background: "#111" }}>
                  <tr>
                    <th style={{ ...S.th, padding: "8px 4px" }}>User ID</th>
                    <th style={{ ...S.th, padding: "8px 4px" }}>Status</th>
                    <th style={{ ...S.th, padding: "8px 4px" }}>Note</th>
                  </tr>
                </thead>
                <tbody>
                  {syncResults.details.map((res, i) => (
                    <tr key={i} style={{ borderBottom: "1px solid #222" }}>
                      <td style={{ ...S.td, padding: "8px 4px", fontSize: "0.75rem", fontFamily: "monospace" }}>{res.userId}</td>
                      <td style={{ ...S.td, padding: "8px 4px" }}>
                        <span style={{ 
                          fontSize: "0.65rem", 
                          padding: "2px 6px", 
                          borderRadius: 4,
                          background: res.status === "success" ? "rgba(74,222,128,0.1)" : res.status === "not_in_server" ? "rgba(251,191,36,0.1)" : "rgba(248,113,113,0.1)",
                          color: res.status === "success" ? "#4ade80" : res.status === "not_in_server" ? "#fbbf24" : "#f87171",
                          fontWeight: 700
                        }}>
                          {res.status.toUpperCase()}
                        </span>
                      </td>
                      <td style={{ ...S.td, padding: "8px 4px", fontSize: "0.7rem", color: "#888" }}>{res.message || "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
