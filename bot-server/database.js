...
const initialize = async () => {
  ...
  // Add verified column to users if missing
  try {
    const [cols] = await pool.query(`
      SELECT COLUMN_NAME FROM information_schema.COLUMNS
      WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND COLUMN_NAME = 'verified'
    `);
    if (cols.length === 0) {
      await pool.query(`ALTER TABLE users ADD COLUMN verified BOOLEAN DEFAULT FALSE`);
    }
  } catch (error) {
    console.log('Migration note (users.verified):', error.message);
  }
...
const updateUserRoles = async (userId, roles, patreonPremium) => {
  ...
  // Role-based sync for Premium, Leak Protection, and Verified status
  ...
  const hasVerified = verifiedRoleId && Array.isArray(roles) ? roles.includes(verifiedRoleId) : true; // Default to true if no role configured (legacy)

  try {
    const [result] = await pool.query(
      'UPDATE users SET roles = ?, patreon_premium = ?, leak_protection = ?, verified = ? WHERE id = ?',
      [rolesJson, Boolean(patreonPremium), Boolean(hasLeakProtection), Boolean(hasVerified), userId]
    );
