...
/**
 * /leak-restore — Web admin dashboard trigger for the Skript bot's /restore command.
 * Body: { scope: "all" | "editor", editor?: string, requestedBy?: string, requestedByName?: string }
 *
 * The actual forum/post creation is owned by the Skript bot. This endpoint
 * notifies a configured Discord control channel so an admin (or the Skript bot
 * via a listener) can run /restore. We do NOT touch protected resources here.
 */
apiServer.post('/leak-restore', async (req, res) => {
  try {
    const { scope = 'all', editor = null, requestedBy = null, requestedByName = null } = req.body || {};
    const controlChannelId = (process.env.LEAK_RESTORE_CHANNEL_ID || '').trim();

    const summary = scope === 'editor'
      ? `\`/restore editor ${editor}\``
      : '`/restore all`';
    const requestedTag = requestedByName ? `${requestedByName}${requestedBy ? ` (<@${requestedBy}>)` : ''}` : 'admin';

    if (controlChannelId && client?.channels?.cache) {
      try {
        const ch = await client.channels.fetch(controlChannelId).catch(() => null);
        if (ch && ch.isTextBased && ch.isTextBased()) {
          await ch.send({
            content: `Resource restore requested by **${requestedTag}**: please run ${summary} (web dashboard trigger). Protected resources are excluded.`,
            allowedMentions: { parse: [] }
          });
        }
      } catch (e) {
        console.warn('[leak-restore] Could not send control channel message:', e?.message || e);
      }
    } else {
      console.log('[leak-restore] No LEAK_RESTORE_CHANNEL_ID set; restore requests will not be auto-announced. Run', summary, 'manually.');
    }

    res.json({ success: true, message: `Restore request sent. Run ${summary} in Discord to execute.`, scope, editor });
  } catch (err) {
    console.error('[leak-restore] Failed:', err?.message || err);
    res.status(500).json({ error: 'Failed to handle restore request' });
  }
});
