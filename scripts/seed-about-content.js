...
async function main() {
  ...
  try {
    ...
    try { await pool.execute('ALTER TABLE resources_items ADD COLUMN description TEXT;'); } catch(e) {}
    try { await pool.execute("ALTER TABLE resources_items ADD COLUMN status VARCHAR(50) DEFAULT 'Completed';"); } catch(e) {}
    try { await pool.execute('ALTER TABLE resources_items ADD COLUMN counts_for_payout BOOLEAN DEFAULT true;'); } catch(e) {}
    try { await pool.execute('ALTER TABLE resources_items ADD COLUMN is_featured BOOLEAN DEFAULT false;'); } catch(e) {}
  } finally {
    await pool.end();
  }
